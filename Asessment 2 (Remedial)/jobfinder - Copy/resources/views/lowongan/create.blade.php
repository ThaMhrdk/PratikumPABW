...
<div class="card">
    <div class="card-header">
        <h3><i class="fas fa-briefcase"></i> Form Lowongan Pekerjaan</h3>
    </div>
    <div class="card-body">
        <form id="createForm" action="{{ route('lowongan.store') }}" method="POST">
            @csrf
            ...
            <div class="form-group">
                ...
                <input type="text" name="perusahaan" id="perusahaan" class="form-control @error('perusahaan') is-invalid @enderror" 
                       value="{{ old('perusahaan') }}" placeholder="Contoh: PT Teknologi Bandung" required>
                @error('perusahaan')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" name="lokasi_kerja" id="lokasi_kerja" class="form-control @error('lokasi_kerja') is-invalid @enderror" 
                       value="{{ old('lokasi_kerja') }}" placeholder="Contoh: Soreang, Kab. Bandung" required>
                @error('lokasi_kerja')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" name="gaji" id="gaji" class="form-control @error('gaji') is-invalid @enderror" 
                       value="{{ old('gaji') }}" placeholder="Contoh: 5000000" min="0">
            ...
            <div class="form-group">
                ...
                <textarea name="deskripsi" id="deskripsi" class="form-control @error('deskripsi') is-invalid @enderror" 
                          rows="5" placeholder="Jelaskan deskripsi pekerjaan, kualifikasi yang dibutuhkan, dll.">{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
